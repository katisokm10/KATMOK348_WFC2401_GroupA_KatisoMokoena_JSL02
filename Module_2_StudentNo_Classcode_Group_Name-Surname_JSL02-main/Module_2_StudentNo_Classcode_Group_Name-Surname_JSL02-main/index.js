...
const addNewGoal = () => {
    const goalInput = document.querySelector('#goalInput').value.trim(); // Trim whitespace from input value
    ...
    existingGoals.forEach(existingGoal => {
        if (existingGoal.textContent === goalInput) {
            alert('This goal already exists!');
            isDuplicate = true;
        }
    });

    // If not a duplicate and not empty, add the goal
    if (!isDuplicate && goalInput !== '') {
        const newGoal = document.createElement('li');
        newGoal.textContent = goalInput;
        goalList.appendChild(newGoal);

        // Add event listener to remove the goal when clicked
